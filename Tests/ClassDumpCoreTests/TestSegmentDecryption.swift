...
struct BlowfishLegacyTests {
  ...
  func roundTrip() {
    ...
    // Test values
    var left: UInt32 = 0x12345678
    var right: UInt32 = 0x9ABCDEF0
  ...
  func longKeySupport() {
    ...
    var left: UInt32 = 0xDEADBEEF
    var right: UInt32 = 0xCAFEBABE

    // Should work without crashing
    blowfish.decrypt(left: &left, right: &right)

    #expect(left != 0xDEADBEEF || right != 0xCAFEBABE)
  }
  ...
  func consistentDecryption() {
    ...
    var left1: UInt32 = 0x11111111
    var right1: UInt32 = 0x22222222
    var left2: UInt32 = 0x11111111
    var right2: UInt32 = 0x22222222
...
struct SegmentDecryptorTests {
  ...
  func magicConstants() {
    ...
    #expect(SegmentDecryptor.Magic.aes == 0xc2286295)
    #expect(SegmentDecryptor.Magic.blowfish == 0x2e69cf40)
  }
  ...
  func detectUnknown() {
    ...
    data.withUnsafeMutableBytes { buffer in
      buffer.storeBytes(of: unknownMagic.littleEndian, toByteOffset: 3 * SegmentDecryptor.pageSize, as: UInt32.self)
    }
  ...
  func decryptNone() throws {
    ...
    // Set none magic
    data.withUnsafeMutableBytes { buffer in
      buffer.storeBytes(of: SegmentDecryptor.Magic.none.littleEndian,
  ...
  func unsupportedEncryption() {
    var data = Data(count: 4 * SegmentDecryptor.pageSize)
    let unknownMagic: UInt32 = 0xDEADBEEF
    data.withUnsafeMutableBytes { buffer in
      buffer.storeBytes(of: unknownMagic.littleEndian,
                        toByteOffset: 3 * SegmentDecryptor.pageSize,
                        as: UInt32.self)
    }
...
struct SegmentEncryptionTypeTests {
  ...
  func initFromMagic() {
    ...
    #expect(SegmentEncryptionType(magic: 0xc2286295) == .aes)
    #expect(SegmentEncryptionType(magic: 0x2e69cf40) == .blowfish)
