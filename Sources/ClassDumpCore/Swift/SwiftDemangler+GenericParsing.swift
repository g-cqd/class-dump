...
extension SwiftDemangler {
    ...
    /// Parse literal "Never" type (used in Task failure type).
    ///
    /// In Swift mangling, `Never` sometimes appears as a literal string
    /// rather than a mangled pattern.
    private static func parseLiteralNever(_ input: Substring) -> (String, Substring)? {
        guard input.hasPrefix("Never") else { return nil }
