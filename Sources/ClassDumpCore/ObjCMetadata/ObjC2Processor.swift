...
public final class ObjC2Processor: @unchecked Sendable {
  ...
  private func loadImageInfo() throws -> ObjC2ImageInfo? {
    guard let section = findSection(segment: "__DATA", section: "__objc_imageinfo")
        ?? findSection(segment: "__DATA_CONST", section: "__objc_imageinfo") else {
      return nil
    }
  ...
  private func loadProtocols() throws -> [ObjCProtocol] {
    guard let section = findSection(segment: "__DATA", section: "__objc_protolist")
        ?? findSection(segment: "__DATA_CONST", section: "__objc_protolist") else {
      return []
    }
  ...
  private func loadProtocol(at address: UInt64) throws -> ObjCProtocol? {
    ...
    // Load methods
    for method in try loadMethods(at: rawProtocol.instanceMethods, extendedTypesAddress: rawProtocol.extendedMethodTypes) {
      proto.addInstanceMethod(method)
    }

    for method in try loadMethods(at: rawProtocol.classMethods, extendedTypesAddress: rawProtocol.extendedMethodTypes) {
  ...
  private func loadClasses() throws -> [ObjCClass] {
    guard let section = findSection(segment: "__DATA", section: "__objc_classlist")
        ?? findSection(segment: "__DATA_CONST", section: "__objc_classlist") else {
      return []
    }
  ...
  private func loadCategories() throws -> [ObjCCategory] {
    guard let section = findSection(segment: "__DATA", section: "__objc_catlist")
        ?? findSection(segment: "__DATA_CONST", section: "__objc_catlist") else {
      return []
    }
  ...
  private func loadSmallMethods(at listAddress: UInt64, listHeader: ObjC2ListHeader) throws -> [ObjCMethod] {
    ...
    for i in 0..<listHeader.count {
      ...
      if let selectorRefOffset = fileOffset(for: selectorRefVMAddr) {
        ...
        let rawSelectorPtr = byteOrder == .little ? try selectorCursor.readLittleInt64() : try selectorCursor.readBigInt64()
  ...
  private func loadInstanceVariables(at address: UInt64) throws -> [ObjCInstanceVariable] {
    ...
    for _ in 0..<listHeader.count {
      ...
      if offsetAddr != 0, let offsetPtr = fileOffset(for: offsetAddr) {
        ...
        if is64Bit {
          let value = byteOrder == .little ? try offsetCursor.readLittleInt64() : try offsetCursor.readBigInt64()
          actualOffset = UInt64(UInt32(truncatingIfNeeded: value))
        } else {
          let value = byteOrder == .little ? try offsetCursor.readLittleInt32() : try offsetCursor.readBigInt32()
