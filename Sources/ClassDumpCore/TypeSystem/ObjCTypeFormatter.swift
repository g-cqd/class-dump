...
public struct ObjCTypeFormatter: Sendable {
    ...
    ///
    /// **Note**: Since `StructureRegistry` is now an actor, sync formatting methods cannot
    /// use it directly. Pre-resolve types using `await registry.resolve()` before formatting,
    /// or use async formatting workflows.
    ...
    ///
    /// **Note**: Since `MethodSignatureRegistry` is now an actor, sync formatting methods
    /// cannot use it directly. Pre-resolve block signatures before formatting, or use
    /// async formatting workflows.
    ...
    private func enhanceBlockType(_ type: ObjCType, selector: String, argumentIndex: Int) -> ObjCType {
        ...
        // Note: MethodSignatureRegistry is now an actor. To use registry-based enhancement,
        // pre-resolve block signatures asynchronously before formatting:
        //   let richTypes = await registry.blockSignature(forSelector:argumentIndex:)
        //   let enhancedType = ObjCType.block(types: richTypes)
        // Then format the enhanced type.
    ...
    private func formatStructOrUnion(
        ...
        // Note: StructureRegistry is now an actor. To resolve forward-declared structures,
        // pre-resolve types asynchronously before formatting:
        //   let resolved = await registry.resolve(forwardType)
        // Then pass the resolved type to the formatter.
        // For now, forward declarations remain unresolved in sync formatting.
