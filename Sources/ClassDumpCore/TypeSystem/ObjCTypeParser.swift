  ...
  private func parseTypeInternal(inStruct: Bool) throws -> ObjCType {
    ...
    if case .char("@") = lookahead {
      ...
      if case .quotedString(let str) = lookahead {
        ...
        let shouldParseAsClassName = !inStruct ||
          str.first?.isUppercase == true ||
          !isTokenInTypeStartSet(charToken(lexer.peekChar))
        ...
        if shouldParseAsClassName {
          ...
          if let protocolStart = str.firstIndex(of: "<"),
            ...
            let protocols = str[protocolRange].split(separator: ",").map { String($0).trimmingCharacters(in: .whitespaces) }
